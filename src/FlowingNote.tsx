...
export const FlowingNote = ({
  ...
  const color = useMemo(() => {
    const y =
      colourNameToHex(black ? noteColor.black : noteColor.white) ?? 'white';
    console.log(y);
    const x = parseInt(y, 16) + Math.sin(Math.random() * 3.12) * 120;
    console.log(x);
    return x;
  }, [black, noteColor]);
  ...
  return (
    <group ref={meshRef} position={[0, 0, 0]}>
      ...
      <lineSegments>
        ...
        />
        <lineBasicMaterial linewidth={0.5} color="white" />
      </lineSegments>
    </group>
  );
};
